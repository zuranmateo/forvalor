...
public class SpearmanScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (CompareTag("SpearManRed") && UIbattle != null)
        {
    ...
    private IEnumerator ReturnAndRotate(Vector3 destination, Quaternion targetRotation)
    {
        ...
        while (true)
        {
            ...
            {
                // V agresiveMode == false ho�e gledat kam gre
    ...
    {
        if (CompareTag(spearmanAllyTag) && (other.CompareTag(spearmanEnemyTag)|| other.CompareTag(bowmanEnemyTag)|| other.CompareTag(swordmanEnemyTag) || other.CompareTag(EnemyDoorTag) || other.CompareTag(throneEnemyTag) || other.CompareTag(EnemyBuildingTag)))
        {
            attackable = true;
        }
    }
        ...
        if (tipdamage != null && !dead && tipdamage.AttackedEnemy && CloseEnemy != null)
        {
            ...
            else if (CloseEnemy.CompareTag(swordmanEnemyTag))
            {
                ...
                if (swordman != null && !dead)
                {
                    ...
                    //Debug.Log(swordman.shieldHealth);
            ...
            else if (CloseEnemy.CompareTag(EnemyDoorTag))
            {
                ...
                if(AttackedEnemy == null) return;
                Debug.Log(AttackedEnemy + AttackedEnemy.transform.parent.name);
    ...
    private void attack()
    {
        ...
        canAttack = false;
        StopWalking();
        Invoke(nameof(attackCooldown), 1.2f);
    }
    ...
    /*
    private void CheckObstructionBetweenAgentAndTarget()
    {
        // Raycast origin: dvignemo ray iz telesa navzgor (da ne zadane tla ali nog)
        Vector3 origin = body.transform.position + Vector3.up * 0.5f;
        ...
        Debug.Log("Raycast from: " + origin + " to: " + target.transform.position);

        if (Physics.Raycast(ray, out hit, distance))
        {
            Debug.Log("Raycast hit: " + hit.collider.name + " Tag: " + hit.collider.tag);

            if (hit.collider.CompareTag(spearmanAllyTag))
            {
                // Preverimo, �e je �iva enota (da ne spro�amo na trupla)
                SpearmanScript script = hit.collider.GetComponent<SpearmanScript>();
                if (script != null && !script.dead)
                {
                    animator.SetBool("PersonInWay", true);
                    Debug.Log("Person in way: " + hit.collider.name);
                    return;
                }
            }
        }

        // �e ni� ni zadelo ali ni ally, resetiramo animator
        animator.SetBool("PersonInWay", false);
    }
    */
}
